...
sigma_piL      = NaN(T+1,T+1+S,1);      % Standard error of expected profitability for low-carbon technology
sigma_piH      = NaN(T+1,T+1+S,1);      % Standard error of expected profitability for low-carbon technology
sigma2_uH       = NaN(T+1,T+1+S,1);     % Variance of utilisation error term in time s
sigma2_piL      = NaN(T+1,T+1+S,1);     % Variance of expected profitability for low-carbon technology
sigma2_piH      = NaN(T+1,T+1+S,1);     % Variance of expected profitability for low-carbon technology
...
%% Prices
  ...
  for s=1:S
  gamma_H_e(t,t+s) = (p_E(t)-p_FF_e(s)/xi_F)*xi_H(t);                      %Fossil fuel price constant in expectations in current version
  end
...
%% Uncertainty of Expectations
  for s = 1:S
    ...
    sigma_piL(t,t)          = SigmaMin_piL;
    sigma_piH(t,t)          = SigmaMin_piH;
    if (SigmaMax>=SigmaMin)
    sigma_u_H(t,t+s)        = (sigma_u_H(t,t+s-1) +  r_log_sigma*sigma_u_H(t,t+s-1)*(1-sigma_u_H(t,t+s-1)/(SigmaMax)));
    else
    sigma_u_H(t,t+s)        = 0;  
    end
    if (SigmaMax_piL>=SigmaMin_piL)
    sigma_piL(t,t+s)        = (sigma_piL(t,t+s-1) +  r_log_sigma*sigma_piL(t,t+s-1)*(1-sigma_piL(t,t+s-1)/(SigmaMax_piL))); 
    else
    sigma_piL(t,t+s)        = 0;
    end
    if (SigmaMax_piH>=SigmaMin_piH)
    sigma_piH(t,t+s)        = (sigma_piH(t,t+s-1) +  r_log_sigma*sigma_piH(t,t+s-1)*(1-sigma_piH(t,t+s-1)/(SigmaMax_piH)));
    else
    sigma_piH(t,t+s)        = 0;
    end
    sigma2_uH(t,t+s)      = sigma_u_H(t,t+s)^2;
    sigma2_piL(t,t+s)     = sigma_piL(t,t+s)^2;
    sigma2_piH(t,t+s)     = sigma_piH(t,t+s)^2;
    % Expected unit profits mu_pi
    mu_pi_H(t,t+s)          =  gamma_H_e(t,t+s)*mu_u_H(t,t+s)- dr_H(t);
    ...
    sigma2_pi_H(t,t+s)    = beta^(2*s)*((sigma_u_H(t,t+s)*(gamma_H_e(t,t+s)))^2+(sigma2_piH(t,t+s)*xi_H(t)*mu_u_H(t,t+s))^2+(sigma2_piL(t,t+s)*xi_L(t)*u_L(t))^2+(sigma_piH(t,t+s)*sigma_u_H(t,t+s)*xi_H(t))^2);
...
%% INVESTMENT DECISION     
  ...
  ell_I(t) = (cdf('Normal',0,mu_phi(t),sigma_r_H(t)) ...
      - (mu_phi(t) > sum(lower_bound_pi_h(t,t+1:t+S)))*(mu_phi(t) < sum(upper_bound_pi_h(t,t+1:t+S)))*cdf('Normal',0,sum(lower_bound_pi_h(t,t+1:t+S)),sigma_r_H(t))  ...
      -  (mu_phi(t) > sum(upper_bound_pi_h(t,t+1:t+S)))*cdf('Normal',0,sum(upper_bound_pi_h(t,t+1:t+S)),sigma_r_H(t)))/(cdf('Normal',0,sum(upper_bound_pi_h(t,t+1:t+S)),sigma_r_H(t)) ...
      - cdf('Normal',0,sum(lower_bound_pi_h(t,t+1:t+S)),sigma_r_H(t)));
  end
